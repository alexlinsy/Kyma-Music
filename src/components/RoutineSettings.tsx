          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-[calc(100%-1.5rem)] sm:w-full max-w-xl bg-kyma-panel border border-kyma-text/10 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-2xl overflow-hidden max-h-[85vh] flex flex-col text-kyma-text">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-kyma-text">Daily Routines</h2>
              </div>
              <button onClick={onClose} className="text-zinc-500 hover:text-kyma-text/80"><X size={24} /></button>
            </div>

            <div className="flex-1 overflow-y-auto space-y-4 pr-2 scrollbar-hide">
              {routines.map((r, i) => (
                <div key={i} className="flex items-center gap-3 bg-kyma-text/5 p-4 rounded-2xl border border-kyma-text/10 text-kyma-text">
                  <input type="time" value={r.time} onChange={e => updateRoutine(i, 'time', e.target.value)} className="bg-transparent font-mono text-sm focus:outline-none" />
                  <input type="text" placeholder="Activity" value={r.activity} onChange={e => updateRoutine(i, 'activity', e.target.value)} className="bg-transparent text-sm flex-1 focus:outline-none border-b border-kyma-text/10" />
            ...
            <div className="mt-8 flex gap-3">
              <button onClick={addRoutine} className="flex-1 py-3 bg-kyma-text/5 text-kyma-text/80 rounded-xl font-bold hover:bg-kyma-text/10 transition-all flex items-center justify-center gap-2">
