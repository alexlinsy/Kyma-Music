      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.9, y: 20 }}
            className="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl text-white"
          ...
          >
            ...
            <div className="p-6">
              {isLoading ? (
                ...
                <textarea
                  className="w-full h-64 bg-black/40 p-4 rounded-xl border border-white/10 focus:ring-2 focus:ring-[#0075de] focus:outline-none font-mono text-sm resize-none custom-scrollbar"
                  value={rules}
            ...
            <div className="px-6 py-4 bg-black/20 border-t border-white/5 flex justify-end gap-3">
              ...
              <button onClick={handleSave} disabled={isSaving} className="px-5 py-2 text-sm font-bold bg-[#0075de] rounded-lg hover:bg-[#0065c2] transition-colors flex items-center gap-2 disabled:opacity-50">
